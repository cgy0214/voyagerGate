...
async function toggleAllowLan() {
  ...
  try {
    ...
    toast(next ? '已允许局域网访问（绑定 0.0.0.0）' : '已限制为本机访问（绑定 127.0.0.1）')
  } catch (e) {
    toast(String(e && e.message ? e.message : e), 'err')
  }
}
...
<template>
  <ModalShell title="⚙ 设置" @close="emit('close')">
    ...
    <div class="frow"><label>局域网访问</label>
      <span class="switch" :class="{ on: allowLan }" title="开启=代理绑定 0.0.0.0，局域网设备可经本机转发；关闭=仅本机 127.0.0.1 可访问" @click="toggleAllowLan"><span class="knob"></span></span>
      <span style="color:var(--dim)">{{ allowLan ? '允许（0.0.0.0）' : '仅本机（127.0.0.1）' }}</span>
    </div>
